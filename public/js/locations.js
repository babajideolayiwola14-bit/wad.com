/**
 * Nigeria states/LGAs and select helpers.
...
window.Locations = (function () {
    ...
    function fillStateSelect(selectEl, selectedValue) {
        if (!selectEl) return;
        ...
        Object.keys(nigeriaData).sort().forEach(state => {
            ...
            if (selectedValue && state === selectedValue) option.selected = true;
            selectEl.appendChild(option);
        });
    }

    function fillLgaSelect(stateValue, lgaSelect, selectedLga) {
        if (!lgaSelect) return;
        ...
        if (stateValue && nigeriaData[stateValue]) {
            nigeriaData[stateValue].forEach(lga => {
                ...
                if (selectedLga && lga === selectedLga) option.selected = true;
                lgaSelect.appendChild(option);
            });
        }
    }

    function wireStateLga(stateSelect, lgaSelect) {
        if (!stateSelect || !lgaSelect) return;
        stateSelect.addEventListener('change', () => {
            fillLgaSelect(stateSelect.value, lgaSelect);
        });
    }
